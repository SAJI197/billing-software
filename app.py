...
app.config["SECRET_KEY"] = SECRET_KEY or "change-this-in-production"
...
def bootstrap_database_and_admin():
    db.create_all()
...
def next_number(prefix, current_value=None):
    if current_value:
        return current_value

    if prefix == "INV":
        last = db.session.execute(db.select(func.max(Invoice.id))).scalar() or 0
    elif prefix == "PUR":
        last = db.session.execute(db.select(func.max(Purchase.id))).scalar() or 0
    elif prefix == "PAY":
        last = db.session.execute(db.select(func.max(PaymentMade.id))).scalar() or 0
    elif prefix == "REC":
        last = db.session.execute(db.select(func.max(Receipt.id))).scalar() or 0
    else:
        last = db.session.execute(db.select(func.max(CreditNote.id))).scalar() or 0
...
def invoice_new():
    if request.method == "POST":
        ...
        inv = Invoice(
            invoice_no=request.form.get("invoice_no") or next_number("INV"),
...
def purchases():
    if request.method == "POST":
        ...
        row = Purchase(
            doc_no=request.form.get("doc_no") or next_number("PUR"),
...
def purchase_pdf(row_id):
    ...
    pdf = simple_pdf(
        ...
        row.doc_date,
        row.supplier.name,
        [row.supplier.address, f"TRN: {row.supplier.trn}" if row.supplier.trn else ""],
        [
...
def payments():
    if request.method == "POST":
        row = PaymentMade(
            voucher_no=request.form.get("voucher_no") or next_number("PAY"),
...
def receipts():
    if request.method == "POST":
        row = Receipt(
            voucher_no=request.form.get("voucher_no") or next_number("REC"),
            voucher_date=datetime.strptime(request.form["voucher_date"], "%Y-%m-%d").date(),
            customer_id=int(request.form["customer_id"]),
            amount=to_decimal(request.form["amount"]),
            reference=request.form.get("reference", ""),
...
def credit_notes():
    if request.method == "POST":
        ...
        row = CreditNote(
            credit_no=request.form.get("credit_no") or next_number("CRN"),
